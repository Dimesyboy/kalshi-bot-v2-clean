...
def get_best_no_legs(game_filter=None, n=10, yes_min=0.05, yes_max=0.70):
    """
    ...
    try:
        ...
        _r = _req.get('https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/scoreboard',
    ...
    MIN_THRESHOLDS = {'KXMLBHR': 1, 'KXMLBHIT': 1, 'KXMLBTOTAL': 6}

    scored = []
    for m in all_m:
        yb = float(m.get('yes_bid_dollars',0) or 0)
        if not (yes_min <= yb <= yes_max): continue
        if float(m.get('yes_ask_size_fp',0) or 0) < 10: continue
    ...
    # ── Supplement with MLB run total UNDER legs if strong model edge ──
    try:
        from data.mlb_totals import get_tonight_mlb_edges
        from core.kalshi_client import _signed_get as _sg

        mlb_edges  = get_tonight_mlb_edges()
        total_data = _sg('/trade-api/v2/markets?series_ticker=KXMLBTOTAL&limit=200&status=open')
        total_mkts = {m['ticker']: m for m in total_data.get('markets',[])}

        for g in mlb_edges:
            kalshi_line = g.get('kalshi_line')
            exp_total   = g.get('exp_total', 0)
            if not kalshi_line or not exp_total: continue
            edge_pts = round(exp_total - kalshi_line, 1)
            if edge_pts >= -1.5: continue  # need >1.5 run UNDER edge

            # Find the matching Kalshi ticker closest to YES=0.50
            game_code = g['away_team'] + g['home_team']
            game_mkts = [(float(t.split('-')[-1]), t) for t in total_mkts
                         if game_code in t and t.split('-')[-1].replace('.','').isdigit()]
            ...
            if 0.25 <= yb <= 0.75 and ticker not in {l['ticker'] for l in legs}:
                from data.mlb_totals import edge_to_confidence_mlb
                total_leg = {
                    'ticker':     ticker,
                    'player':     f"{g['away_team']}@{g['home_team']} UNDER {kalshi_line}",
                    'yes_bid':    yb,
                    'conf':       edge_to_confidence_mlb(edge_pts) / 100,
                    'ask_size':   float(m.get('yes_ask_size_fp',0) or 0),
                    'oi':         float(m.get('open_interest_fp',0) or 0),
                    'vol':        float(m.get('volume_24h_fp',0) or 0),
                    'buy_pressure': 0.5,
                    'smart_money':  0,
                    'composite':  round(abs(edge_pts) * 0.5, 3),
                }
                legs.append(total_leg)
                log.info(f'Added MLB UNDER leg: {game_code} {kalshi_line} edge={edge_pts:+.1f} conf={total_leg["conf"]:.2f}')
    except Exception as _te:
        log.debug(f'MLB total leg supplement failed: {_te}')
            ...
            try:
                ...
                try:
                    ...
                    record_order(
                        ...
                        ticker=_f.get('ticker',''), strategy='mlbot_no',
